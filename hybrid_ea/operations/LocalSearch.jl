...
function insert_var!(routes, constrained_variables, patients, travel_time_table, depot)
    ...
    while size(patient_list, 1) > 0
        ...
        while i <= size(patient_list, 1)
            ...
            closest_neighbors = get_route_neighborhood(3, centroids, 0, patients[patient_id]) # Allows more than just 2 route neighbors, which could be interesting to look at 
