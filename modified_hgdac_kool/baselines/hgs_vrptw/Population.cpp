	...
	Individual* Population::getBinaryTournament()
	{
		...
		// Calculate mean and standard deviation for the two individuals
		double meanFitness = (individual1->biasedFitness + individual2->biasedFitness) / 2.0;
		double variance = (pow(individual1->biasedFitness - meanFitness, 2) + 
						pow(individual2->biasedFitness - meanFitness, 2)) / 2.0;
		double stdDev = sqrt(variance);
		
		// Apply sigma scaling
		const double c = 2.0; // scaling constant
		double scaledFitness1 = std::max(individual1->biasedFitness - (meanFitness - c * stdDev), 0.0);
		double scaledFitness2 = std::max(individual2->biasedFitness - (meanFitness - c * stdDev), 0.0);
		
		// Calculate selection probabilities
		double totalScaledFitness = scaledFitness1 + scaledFitness2;
		double probability1, probability2;
		
		if (totalScaledFitness > 0) {
			probability1 = scaledFitness1 / totalScaledFitness;
			probability2 = scaledFitness2 / totalScaledFitness;
		} else {
			// If both scaled fitnesses are 0, use equal probability
			probability1 = probability2 = 0.5;
		}
		
		// Stochastic Universal Sampling with just two individuals
		// For two individuals, SUS simplifies to comparing a random value with probability1
		static std::random_device rd;
		static std::mt19937 gen(rd());
		std::uniform_real_distribution<double> dist(0.0, 1.0);
		double randomPoint = dist(gen);
		
		// Return the selected individual
		if (randomPoint < probability1) {
			return individual1;
		} else {
			return individual2;
		}
